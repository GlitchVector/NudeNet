    ...
    def load_model(self):
        ...
        try:
            try:
                ...
                # Create a simplified model directly without requiring ultralytics
        ...
        try:
            with torch.no_grad():
                ...
                if self.model is None:
                    ...
                    output = torch.zeros((1, 6, 5 + len(LABELS)), device=self.device)
                    # (Same fixed detection code as above)
    ...
    def detect_batch(self, image_paths: List[Union[str, np.ndarray]], batch_size: int = 4) -> List[List[Dict[str, Any]]]:
        ...
        for i in range(0, len(image_paths), batch_size):
            ...
            for img_path in batch:
                ...
                if input_tensor is None:
                    # If we can't read the image, use a dummy tensor but remember original dimensions
                    ...
                    logger.warning(f"Could not read image, using dummy input")
                else:
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    # Handle various cases (similar to detect method)
                    if self.model is None:
                        ...
                        logger.info("Model forward method used successfully for batch")
                    elif hasattr(self.model, 'predict'):
                        outputs = self.model.predict(stacked_input)
                        logger.info("Model predict method used successfully for batch")
                    else:
